...
private ElasticacheServersConfig elasticacheConfig;
  ...
  private SentinelServersSelfConfig sentinelSelfConfig;
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (singleConfig != null) {
      ...
      return;
    } else if (sentinelSelfConfig != null) {
      initSentinelConfig(sentinelSelfConfig);
      return;
    } else {
      throw new Exception(
          "cluster servers config、sentinel servers config、master/slave servers、single server config、elasticache replication group servers config etc. must be configured with a");
    }
  ...
  private void initSentinelConfig(SentinelServersSelfConfig sentinelSelfConfig) throws Exception {
    ...
    List<String> list = sentinelSelfConfig.getAddresses();
    String[] listAddress = list.toArray(new String[list.size()]);
    sentinelServersConfig.setMasterName(sentinelSelfConfig.getMasterName());
    sentinelServersConfig.addSentinelAddress(listAddress);
  ...
  private void initMasterSlaveConfig(MasterSlaveServersConfig masterSlaveServersConfig)
      throws Exception {
    checkClusterServersConfig();
  ...
  @SuppressWarnings("deprecation")
private void initElasticacheConfig(ElasticacheServersConfig elasticacheServersConfig)
      throws Exception {
    checkClusterServersConfig();
    checkMasterSlaveServersConfig();
    checkSentinelServersConfig();
...
public void setElasticacheConfig(ElasticacheServersConfig elasticacheConfig) {
  ...
  public SentinelServersSelfConfig getSentinelSelfConfig() {
    return sentinelSelfConfig;
  }

  public void setSentinelSelfConfig(SentinelServersSelfConfig sentinelSelfConfig) {
    this.sentinelSelfConfig = sentinelSelfConfig;
  }
